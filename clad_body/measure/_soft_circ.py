...
Calibration (100-body dataset, random seed 42):
    ...
    stomach:   A = 1.0, B = 0.0 (uncalibrated) — validated on 100-body sample:
               MAE 0.93 cm, P95 2.83 cm, max 3.61 cm, bias −0.30 cm
"""
...
# Stomach: uncalibrated — will be calibrated after validation on the 100-body dataset.
STOMACH_A, STOMACH_B = 1.0, 0.0
...
STOMACH_Z_UPPER_FRAC = 1.00      # cap search at hip_z + frac * (waist_z - hip_z)
                                 # — 1.0 = up to waist_z, matches non-diff exactly
...
def measure_stomach_soft(model, verts):
    ...
    **Algorithm** (single soft_circumference call):
    ...
    hz = hip_z(model, verts_zup)
    # Upper bound is capped below the natural waist to avoid the anterior
    # lower-ribcage bulge that can compete with the belly tip.
    z_upper = hz + STOMACH_Z_UPPER_FRAC * (waist_z(model, verts_zup) - hz)

    v = verts_zup[0]  # (V, 3)
    y = v[:, 1]
    z = v[:, 2]

    # Hard Z-mask with 2 cm buffer on each side — gates out bust / foot /
    ...
    # z_upper boundaries (for bodies where these anchors move).
    ...
    raw_cm = stomach_m * 100
    return {"stomach_cm": STOMACH_A * raw_cm + STOMACH_B}
